...
namespace StorageType.Passthrough.IO {
    ...
    internal static class NameMatcher {
        ...
        public static bool Match(string expression, string name, bool casesensitive) {
            ...
            var areNextExpressionCharsAllNullMatchers = expression.Length > 0 && !string.IsNullOrEmpty(nextExpressionChars) && nextExpressionChars.All(x => CharsThatMatchEmptyStringsAtEnd.Contains(x));
            var isNameCurrentCharTheLast = ni == name.Length;
            return (ei == expression.Length && isNameCurrentCharTheLast) || (isNameCurrentCharTheLast && areNextExpressionCharsAllNullMatchers);
        }
    }
}